...
public interface ProductDao extends JpaRepository<Product, Integer> {
	...
	// JDBC executeQuery() for "SELECT" and executeUpdate() for "INSERT, DELETE or UPDATE" sending SQL
	@Modifying
	@Query("update Product set price = :pr where id = :id")
	void updateProductPrice(@Param("pr") double price, @Param("id") int id);
}

// Spring boot using libraries will generate class implementing this interface [ @Repository ]